...
- FR-9.14 **状态栏必须有「关于」**:显示①应用版本(CFBundleShortVersionString)②UI 构建(DigestUIBuildID)③**引擎版本(当前)**——必须向引擎现问(`settings.release_version`),不能读报告里的 `release_version`,那是"生成报告时的版本"④今天的报告由哪个引擎版本生成⑤引擎路径、应用路径、数据目录。这样任何版本错配(应用/引擎/报告三者)都能被一眼看出。
