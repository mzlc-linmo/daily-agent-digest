...
- G3 ~~日报自动上报到团队共享飞书群,并带成员身份标识。~~(已废弃;改为:客户端导出 Markdown + 保留提交接口,服务端待重做)
...
- FR-2.5 **汇总只用「提示词 + 最终结果」(已确认并实现)**:工作总结算的是"要做什么"和"结果是什么",过程(工具调用、网页搜索、代码、命令输出、内部推理)不参与运算。抽取在**采集阶段、截断之前**完成,规则按来源结构化,不做任何关键词匹配:

  | 来源 | 用户提示词 | AI 最终文本 | 交付物 |
  | --- | --- | --- | --- |
  | codex | `item_type = userMessage` → `content[].text` | `agentMessage` → `text` | — |
  | dsh | `type = user/message` → `data.content[].text` | `assistant/message` → `data.message.content[].text`(忽略 `reasoning`/`tool-call` 分片) | `deliverables/presented` → `data.files[]` 的 path + description |
  | pi | `type = message` 且 `message.role = user` → `content[].text` | `role = assistant` → `content[].text`(忽略 `thinking`/`toolCall`) | — |

  其余类型(`reasoning`/`functionCallOutput`/`commandExecution`/`mcpToolCall`/`tool/call`/`tool/result`/`step/*`/`turn/*`/`toolResult` 等)一律作为过程记录丢弃,只统计条数。`DIGEST_CONTEXT_INCLUDE_PROCESS=1` 可回退到"过程也送、但排在最后一层"。
- FR-2.6 **先抽取、后截断**:单条记录必须先解析出结构化正文,再对正文应用 `MESSAGE_CHAR_MAX`(4000)上限。任何"先截断整条 JSON 再解析"的做法都会产生半个 JSON 而丢失内容(见 D-27)。
- FR-2.7 **预算与配额**:抽取后的文本按 `提示词 → 最终文本 → 交付物(→ 过程)` 分层,各来源之间轮转取样,任一来源的体量都不能把别的来源挤掉;单次请求上下文上限 **90,000 字符**;统计写入 `coverage.context`,并输出人类可读的 `coverage_note`(同时写入 `state.coverage_note`)显示在报告窗口底部。
...
- FR-9.14 **状态栏必须有「关于」,只显示一个版本号**:弹窗正文只有 `当前版本:<版本>` 一行,不显示 UI 构建、引擎版本、报告版本或任何路径。诊断信息(引擎/报告版本、引擎与应用路径、数据目录)只写入调试日志。版本取应用包版本(CFBundleShortVersionString)。按钮为「检查最新版本」与「关闭」。

- FR-9.15 **可主动检查最新版本**:「关于」提供「检查最新版本」按钮,**只有用户点击后才联网**(`DIGEST_RELEASES_API`,默认公共发布仓库的 releases/latest),判定三种结果:有新版本可用 / 已是最新 / 开发构建无法比较。应用启动时不得自动联网检查。
...
| D-6 | 版本可追溯不完整:引擎调度路径不注入 `DIGEST_RELEASE_VERSION`(仅托盘 plist 注入);二进制未内嵌版本 | NFR-9 | **已修复**:`install.sh` 写入的 `run.sh` 现在导出安装时的 release tag;新增「关于」面板显示应用/UI/引擎/报告四个版本与路径,并支持检查最新版本(A29) |
...
| D-27 | **采集时先截断整条 JSON 再解析**:每 条记录被 `[:4000]` 截断,dsh 的 65 条 `assistant/message` 有 **54 条变成半个 JSON 无法解析**,最终文本根本取不到,而且这些碎片一直作为"正文"送进模型 | FR-2.6 / A28 | **已修复**:改为先结构化抽取正文、再对正文应用 4000 字符上限(回归测试用超长记录断言) |
| D-26 | **上下文按采集顺序截断,真实工作被整体挤掉**(已由 FR-2.5 的结构化抽取取代):实测 281 条事件只有 34 条进得去且全部来自 codex,deepseek-harness 的 214 条 100% 缺席;送进去的前 12 条全是 `functionCallOutput` 自动化输出,于是日报只产出自动化工作项 | FR-2.5 / A27 | **已修复**:本地排除自动化 + 按信号分层 + 来源轮转 + 90k 预算。同一份数据修复后:326 条 → 排除自动化 46 条 → 送模型 158 条(codex 32 + dsh 126),日报产出 5 项真实工作 |
...
13. 实测(2026-09-13,真实 `$HOME`):只保留提示词/最终文本/交付物后,当天入库 **101 条 / 18,470 字符**,丢弃过程 **375 条**(codex 70 + dsh 305);修复前送模型 158 条 / 89,798 字符,修复后 **62 条 / 14,662 字符**,且无截断、无来源饥饿。日报从"3 项自动化"变为"6 项真实工作"。
14. 修复后 `dsh` 的 65 条 `assistant/message` 截断数从 54 降为 0;fixture 已改用与客户端一致的真实落盘结构(含过程记录),开发环境因此走的是生产抽取路径。
