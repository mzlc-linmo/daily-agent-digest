...
- FR-9.14 **状态栏必须有「关于」,且只显示关键信息**:默认只有一行 `版本:<应用版本>`;**只在真正不一致时**追加 `引擎:<版本>(与应用不一致)` / `今天的报告由:<版本>(与当前引擎不同)`。引擎版本必须向引擎现问(`settings.release_version`),不能读报告里的 `release_version`(那是"生成报告时的版本")。UI 构建、引擎路径、应用路径、数据目录等诊断信息**不进弹窗版面**,通过「复制详细信息」写入剪贴板(同时写入调试日志)。
