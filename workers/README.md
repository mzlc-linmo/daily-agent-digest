...
1. 打开表单填写(只问「申请标题 / 申请人 / 账号」,状态、Key 等字段已隐藏):
   `https://<你的租户>.feishu.cn/share/base/<form_token>`
2. **约 1 分钟内**,该行的「Key」列会自动出现一把随机密钥(由每分钟运行的定时任务生成),
   同时「状态」变为 `已签发`。
3. 成员复制这一格的 Key,填进 App 的「设置」(提交地址 + API Key)。

撤销:把该行的「状态」改成 `已撤销`,定时任务会在 1 分钟内把对应 Key 停用。

> 实现:Worker 的 `scheduled()` 每分钟轮询申请表 —— 未签发的行 → 生成随机 Key、存 KV(只存哈希)、
> 把**明文写回该行**;状态为 `已撤销` 的行 → 停用对应 Key。表就是控制面。

> ⚠️ **安全提醒**:明文 Key 就在申请表里,能打开这张表的人就能看到别人的 Key。
> 建议在飞书里给该表开启**高级权限**并设置「仅可查看自己相关的记录」(按「申请人」列),
> 这样每位成员只看得到自己那一行。
