...
Before the single daily LLM pass the engine keeps only what a work summary needs: **the user's prompts, the assistant's final answers and the delivered files**. Tool calls, search results, code, command output and internal reasoning are dropped at collection time by structured field extraction, not by keyword matching. Extraction happens before any truncation, so a long record never loses its text. The shortlisted text is then ranked (prompts, then results, then deliverables), rotated across providers so no source can crowd out another, and capped at 90,000 characters. The counts are recorded in `coverage` and shown at the bottom of the report window.
